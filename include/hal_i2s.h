 ...
 *      The user must query free space in the TX VFIFO buffer to ensure there is enough free space in the TX VFIFO buffer for the write operation.
 ...
 *      Then the RX VFIFO DMA interrupt handler calls the user's callback to notify that the RX VFIFO buffer is available.
 *      User can fetch data from the RX VFIFO buffer again by calling hal_i2s_rx_read() function.
 *
 ...
 *  - Step3:   Call #hal_i2s_setup_tx_vfifo() and #hal_i2s_setup_rx_vfifo() to set memory for I2S TX and RX VFIFO use.
 ...
 *
 *      static uint32_t tx_buffer[TX_BUFFER_LENGTH];
 *      static uint32_t rx_buffer[RX_BUFFER_LENGTH];
 *
 ...
 * @section Features_Chapter Supported features
 *  - The user can set the initial type to #HAL_I2S_TYPE_EXTERNAL_MODE in #hal_i2s_init() function when connecting an external codec component. \n
 *    An interrupt will be triggered when the data is ready to be transferred using the I2S interface. A user defined callback function is called to handle data transfer in the I2S interrupt service routine. \n
 *  - The system architecture of the driver is shown in the diagram below.
...
typedef enum {
    HAL_I2S_TX_MONO_DUPLICATE_DISABLE = 0,  /**< Right channel of TX, send all 0. */
    HAL_I2S_TX_MONO_DUPLICATE_ENABLE  = 1   /**< Right channel of TX, send the same data as the left.  */
} hal_i2s_tx_mode_t;
...
#ifdef HAL_I2S_EXTENDED
/** @brief  This defines the I2S RX callback function prototype.
 ...
 *  @param[in] user_data is a parameter given by user using #hal_i2s_register_rx_vfifo_callback().
 */
typedef void (*hal_i2s_rx_callback_t)(hal_i2s_event_t event, void *user_data);


/** @brief  This defines the I2S TX callback function prototype.
 ...
 *  @param[in] event is the value defined in #hal_i2s_event_t. This parameter is given by the driver to tell the user of data flow processing behavior.
 *  @param[in] user_data is a parameter given by user using #hal_i2s_register_tx_vfifo_callback().
 */
 ...
 *  @param[in] event is the value defined in #hal_i2s_event_t. This parameter is given by the driver to tell the user of data flow processing behavior.
 *  @param[in] user_data is a parameter given by user using #hal_i2s_register_rx_callback().
 */
typedef void (*hal_i2s_rx_callback_t)(hal_i2s_event_t event, void *user_data);
 ...
 *  @param[in] event is the value defined in #hal_i2s_event_t. This parameter is given by the driver to tell the user of data flow processing behavior.
 *  @param[in] user_data is a parameter given by user using #hal_i2s_register_tx_callback().
 */
typedef void (*hal_i2s_tx_callback_t)(hal_i2s_event_t event, void *user_data);
#endif
 ...
 * @param[in] tx_callback is a callback function to control data transmission.
 * @param[in] user_data is a user defined input data.
 * @return    #HAL_I2S_STATUS_INVALID_PARAMETER, if the tx_callback is invalid. \n
 *              #HAL_I2S_STATUS_OK, if the operation is successful.
 * @note      Send I2S TX events to user-defined task and handle the data transfer there, to avoid longer performances in the NVIC ISR.
 */
 ...
 * @param[in] rx_callback is a callback function for receive data control.
 * @param[in] user_data is a user defined input data.
...
/**
 * @brief     This function registers the callback function for input data.
 * @param[in] rx_callback is a pointer to the callback function for receive data control.
 * @param[in] user_data is a user defined input data.
 * @return    #HAL_I2S_STATUS_INVALID_PARAMETER, if the rx_callback is invalid. \n
 *              #HAL_I2S_STATUS_OK, if the operation is successful.
 * @note      Send I2S RX events to user-defined task and handle the data transfer there, to avoid longer performances in the NVIC ISR.
 */
