...
#endif
#define CFG_CONNSYS_SIMULATION_EN               0
...
#if (CFG_CONNSYS_SIMULATION_EN == 1)
/**
 * This is a simulation function.
 *
 * It keeps the regs and len in connsys_driver. The function connsys_cr_read
 * checks 'len'. If 'len' is larger than 0, connsys_cr_read will then reply
 * caller 'len' when reg is requested.
 */
void connsys_set_rx_sim(uint32_t reg, uint32_t len);
#endif /* (CFG_CONNSYS_SIMULATION_EN == 1) */
...
void connsys_tx_flow_control_update_free_page_cnt(enhance_mode_data_struct_t *p_enhance_mode_data);
...
void connsys_tx_flow_control_set_reserve_page_by_cr();
