...
#ifdef N9_DUAL_INF_READY
#define MAX_INTERFACE_NUM 2
#else /* N9_DUAL_INF_READY */
#define MAX_INTERFACE_NUM 1
#endif /* !N9_DUAL_INF_READY */
...
extern char             g_scan_ssid[];
extern unsigned int     g_scan_ssid_len;
...
#if 0 // move to connsys_profile.h
...
int wifi_scan_table_alloc(uint8_t table_size);
void wifi_scan_table_free();
SCAN_ARRAY_ITEM *wifi_scan_table_array();

int wifi_api_set_event_handler(uint8_t enabled, wifi_event_t idx, wifi_event_handler_t handler);
int wifi_api_event_trigger(wifi_event_t idx, uint8_t *address, unsigned int length);
void BssCipherParse(
	uint16_t CapabilityInfo,
	unsigned char *auth_mode,
	unsigned char *encrypt_mode,
...
void wifi_set_supplicant_ready(void);
