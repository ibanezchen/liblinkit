...
void fw_download();
int32_t connsys_patch_download();
int32_t connsys_fw_direct_download(void);
uint8_t connsys_fw_download_CmdAddressLenReq_event_check(uint8_t *buffer);
uint32_t connsys_create_inband_cmd_txd_rom(uint32_t cmd_len, uint8_t *buffer, enum MT_CMD_TYPE cmd);
int32_t connsys_pda_download(uint32_t dest_addr, uint32_t data_len, uint32_t data_mode, uint32_t data_offset, uint8_t *image);
uint8_t connsys_patch_download_get_semaphore_event_check(uint8_t *buffer, uint8_t *status);
uint32_t connsys_create_inband_cmd_txd_rom(uint32_t cmd_len, uint8_t *buffer, enum MT_CMD_TYPE cmd);
void connsys_fw_download_done(void);
void connsys_low_level_set_sim(uint8_t connsys_port, uint8_t *pkt, uint16_t len);
int32_t connsys_low_level_output(uint8_t *buf, ssize_t len, int32_t inf_num);
void inband_send_cmd(int32_t arg1);
int32_t connsys_low_level_rx_data(uint8_t *buf, ssize_t *len, int32_t port);
int32_t connsys_tx_data(uint8_t *buf, ssize_t len);
int32_t connsys_rx_data(uint8_t *buf, ssize_t len, int32_t port);

int32_t connsys_get_inf_number(uint8_t *buf);
int32_t connsys_intr_enhance_mode_receive_data(void);
void connsys_intr_enhance_mode_dump_last_struct(void);
uint8_t connsys_cli_handler(uint8_t len, char *param[]);

void connsys_get_patch_ver(char *ver);
void connsys_get_n9_fw_ver(char *ver);
uint32_t pse_get_p0_min_resv(void);
