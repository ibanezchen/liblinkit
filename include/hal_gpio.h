 ...
 *       The microcontroller pin connects the periphrial through multiplexer with which the pin can be specified to only one alternate function at a time.
 *   \n
 ...
 *      Inversion function inverses input data of GPIO untill the function is disable, it is usefull when chip is input high active while the connected device is output and low active.  \n
 *      Call hal_gpio_enable_inversion() to inverse the input data of the pin untill the function is disabled by calling hal_gpio_disable_inversion(). \n
 * @}
 * @}
 */
#endif
 ...
 *      To facilitate application use, there are 6 clock-out ports embedded in all GPIO pins and each of them can be configured with appropriate clock frequence to send outside the chip.
 *   \n
 ...
 *    or absorb current from, a connected load. Push-pull outputs are present in TTL and CMOS digital logic circuits and in some types of amplifiers, and are usually impplemented as a complementary pair of transistors, 
 ...
 *            High impedance can prevent target GPIO from electric leakage.
 *            It is necessary to call this function befor further configuration if the pin is in high impedance state.
 * @param[in] gpio_pin specifies pin number to set.
 * @return    To indicate whether this function call is successful or not, for example:
 *            If the return value is #HAL_GPIO_STATUS_OK, it means success;
 *            If the return value is #HAL_GPIO_STATUS_ERROR_PIN, it means a wrong pin number is given, the parameter must be verified;
 ...
 *            the input data of target GPIO will always be inversed untill the inverse function is disabled.
