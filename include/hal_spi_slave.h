 ...
 * @{
 * This section introduces the SPI_Slave APIs including terms and acronyms, supported features,
 ...
 * to communicate with an SPI master interface. This controller is a common SPI slave interface with a self-defined specification
 ...
 * - \b Support \b self-defined \b commands. \n
 *  - Poweron command:
 *    This should be the first command the SPI master sends to the SPI slave during a transaction. The length of each poweron command is 1 byte.
 *  - Poweroff command:
 *    This should be the last command the SPI master sends to the SPI slave during a transaction. The length of each poweroff command is 1 byte.
 *  - Configure write command:
 *    Use this command to configure write parameters. The message includes a configure write command(1 byte), addr_id(4 bytes) and
 ...
 *    sure different SPI slave devices have different addr_id in one system, data_length is the length of transferred data.
 *  - Write data command:
 *    This command is used to write data to SPI slave device. The message content includes a write data command(1 byte), transfer_length(4 bytes)
 *    and the message length is (1+transfer_length) bytes, so that slave select signal stays valid for the SPI master to finish the transaction.
 ...
 *    This command is used to configure read parameters. The message includes a configure read command(1 byte), addr_id(4 bytes) and
 *    (data_length-1)(4 bytes). The total length of this command is 9 bytes, addr_id is used to distinguish between different SPI slave devices, user needs to make
 *    sure different SPI slave devices have different addr_id in one system, data_length is the length of transferred data.
 *  - Read data command:
 *    This command is used to read data from the SPI slave device. The message content includes a read data command(1 byte), transfer_length(4 bytes)
 *    and the message length is (1+transfer_length) bytes, so that slave select signal stays valid for the SPI master to finish the transaction.
 *    Note, that transfer_length must be configured with the same value as the data_length in the read command.
 *  - Read status command:
 *    This command is used for master side to get the SPI slave status. The message content includes a read status command(1 byte) and
 *    the message length is 2 bytes, so that slave select signal stays valid for master to get the status of SPI slave device.
 *  - Write status command:
 *    This command is used for master side to clear the SPI slave device's error status. The message content includes a write status command(1 byte)
 *    and the bitmap of status to be cleared(1 byte), so the message length is 2 bytes.
 *
 * - \b Support \b defined \b command \b pattern. \n
 ...
 *  - The configure write command must be followed by write data command with no other command running in between.
 *  Read command pattern:
 *  - The configure read command must be followed by read data command with no other command running in between.
 ...
 *    the slave. It's possible to have multiple read and write command patterns between the poweron and poweroff commands.
 *  - Except for the read status command and write status command, any other command sent from the SPI master,
 *    should be checked whether the command is accepted by the SPI slave successfully or not by sending a read status command.
 *
 ...
 *  is called in the SPI slave interrupt service routine.
 *
 ...
 *  Then call #hal_spi_slave_register_callback() to register a callback function. Since the SPI slave's behavior is passive, so many actions need to be
 *  done in the callback function. After that call #hal_spi_slave_init() in the callback function, if a poweron interrupt occurred, to apply
 *  basic settings for SPI slave hardware. If the function's return status is #HAL_SPI_SLAVE_STATUS_ERROR, then the SPI slave controller is currently
 *  busy, user should wait until it's idle. Once the transaction is over, always call #hal_spi_slave_deinit() in your callback
 *  function, if a poweroff interrupt occurred, to release the SPI slave resource to make it available for other users. The steps are shown below:
 *  - Step1: Call #hal_pinmux_set_function() to set the GPIO pinmux, if EPT tool didn't been used to configure the related pinmux.
 ...
 *  - Step3: Call #hal_spi_slave_set_command() to set a user-defined command value if user don't want to use the default settings, but need to align with the value used by the SPI master side user.
 *  - Step4: Call #hal_spi_slave_set_early_miso() if user wants to change the SPI timing.
 *  - Step5: Call #hal_spi_slave_init() in the callback function, if poweron interrupt occurred, to initialize a single SPI slave controller.
 ...
 *  - Step8: Call #hal_spi_slave_deinit() in the callback function, if poweroff interrupt occurred, to deinitialize the SPI slave that no longer in use.
 *  - sample code:
 *    @code
 ...
 *           const uint8_t *data = 0x04000000;  // Note: the data buffer address must be non-cacheable address
 *           uint32_t size = 0x00000010;
 *           uint8_t *buffer = 0x04000400;  // Note: the data buffer address must be non-cacheable address
...
typedef enum {
    HAL_SPI_SLAVE_LSB_FIRST = 0,                       /**< Both send and receive data will transfer the LSB first. */
    HAL_SPI_SLAVE_MSB_FIRST = 1                        /**< Both send and receive data will transfer the MSB first. */
...
/** @brief SPI slave send MISO early half SCLK cycle definition. */
typedef enum {
    HAL_SPI_SLAVE_EARLY_MISO_DISABLE = 0,            /**< Disables send MISO early half SCLK cycle feature. */
    HAL_SPI_SLAVE_EARLY_MISO_ENABLE  = 1             /**< Enables send MISO early half SCLK cycle feature. */
} hal_spi_slave_early_miso_t;


/** @brief SPI slave status. */
typedef enum {
    HAL_SPI_SLAVE_STATUS_ERROR             = -4,            /**< SPI slave function error */
    HAL_SPI_SLAVE_STATUS_ERROR_BUSY        = -3,            /**< SPI slave function error busy */
    HAL_SPI_SLAVE_STATUS_ERROR_PORT        = -2,            /**< SPI slave error port. */
    HAL_SPI_SLAVE_STATUS_INVALID_PARAMETER = -1,            /**< SPI slave invalid parameter. */
    HAL_SPI_SLAVE_STATUS_OK                = 0              /**< SPI slave function OK. */
} hal_spi_slave_status_t;

/** @brief SPI slave running status. */
typedef enum {
    HAL_SPI_SLAVE_BUSY              = 0,             /**< SPI slave busy. */
    HAL_SPI_SLAVE_IDLE              = 1              /**< SPI slave not busy, in idle state. */
...
Note: N: means invalid operation.
      Y: means successful operation.
      Abnormal: means error cases.
*******************************************************************/
...
typedef enum {
    ...
    HAL_SPI_SLAVE_FSM_ERROR_PWROFF_AFTER_CR  = 0x02,         /**< SPI slave error case: receive PWROFF command after Config Read command. */
    HAL_SPI_SLAVE_FSM_ERROR_PWROFF_AFTER_CW  = 0x03,         /**< SPI slave error case: receive PWROFF command after Config Write command. */
    HAL_SPI_SLAVE_FSM_ERROR_CONTINOUS_CR     = 0x04,         /**< SPI slave error case: receive continous Config Read command. */
    HAL_SPI_SLAVE_FSM_ERROR_CR_AFTER_CW      = 0x05,         /**< SPI slave error case: receive Config Read command after Config Write command. */
    HAL_SPI_SLAVE_FSM_ERROR_CONTINOUS_CW     = 0x06,         /**< SPI slave error case: receive continous Config Write command. */
    HAL_SPI_SLAVE_FSM_ERROR_CW_AFTER_CR      = 0x07,         /**< SPI slave error case: receive Config Write command after Config Read command. */
    HAL_SPI_SLAVE_FSM_ERROR_WRITE_AFTER_CR   = 0x08,         /**< SPI slave error case: receive Write command after Config Read command. */
    HAL_SPI_SLAVE_FSM_ERROR_READ_AFTER_CW    = 0x09          /**< SPI slave error case: receive Read command after Config Write command. */
} hal_spi_slave_fsm_status_t;


/** @brief SPI slave command enum. */
typedef enum {
    ...
    HAL_SPI_SLAVE_CMD_POWEROFF  = 4,       /**< Poweroff command. */
    HAL_SPI_SLAVE_CMD_POWERON   = 5,       /**< Poweron command. */
...
typedef struct {
    uint32_t timeout_threshold;                      /**< SPI slave timeout threshold setting. */
    hal_spi_slave_bit_order_t bit_order;             /**< SPI slave bit order setting. */
    hal_spi_slave_clock_polarity_t polarity;         /**< SPI slave clock polarity setting. */
    hal_spi_slave_clock_phase_t phase;               /**< SPI slave clock phase setting. */
} hal_spi_slave_config_t;


/** @brief SPI slave transaction status definition. */
typedef struct {
    hal_spi_slave_fsm_status_t fsm_status;           /**< fsm_status. One item from hal_spi_slave_fsm_status_t. */
    uint32_t interrupt_status;                       /**< interrupt_status. The interrupt status of the SPI slave controller. */
 ...
 *          User should register a callback function to use the SPI slave. This function is called in the SPI slave interrupt
 ...
 * @return    #HAL_SPI_SLAVE_STATUS_ERROR the SPI slave port is in use. \n
 *            #HAL_SPI_SLAVE_STATUS_ERROR_BUSY means this port of SPI slave is busy and not available for use; \n
 *            #HAL_SPI_SLAVE_STATUS_ERROR_PORT the spi_port parameter is an invalid port number. \n
 *            #HAL_SPI_SLAVE_STATUS_INVALID_PARAMETER an invalid parameter is given by user. \n
 *            #HAL_SPI_SLAVE_STATUS_OK this function returned successfully.
 * @par       Example
 ...
 * @param[in] spi_port is the SPI slave port number, the value is defined at #hal_spi_slave_port_t.
 * @return    #HAL_SPI_SLAVE_STATUS_ERROR_PORT the spi_port parameter is an invalid port number. \n
 *            #HAL_SPI_SLAVE_STATUS_OK this function returned successfully.
 * @par       Example
 * Sample code please refers to @ref HAL_SPI_SLAVE_Driver_Usage_Chapter.
 ...
 * @param[in] spi_port is the SPI slave port number, the value is defined at #hal_spi_slave_port_t.
 * @param[in] early_miso is the parameter to enable the send miso early half SCLK cycle feature or disable it.
 * @return    #HAL_SPI_SLAVE_STATUS_ERROR_PORT the spi_port parameter is an invalid port number. \n
 *            #HAL_SPI_SLAVE_STATUS_INVALID_PARAMETER an invalid parameter is given by user. \n
 *            #HAL_SPI_SLAVE_STATUS_OK this function returned successfully.
 * @par       Example
